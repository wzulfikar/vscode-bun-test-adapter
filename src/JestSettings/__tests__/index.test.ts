...
describe('getSettings tests', () => {
  ...
  it(`Given a Jest config with a string array for testRegex
    ...
    expect(settings.configs[0].testRegex).toStrictEqual([
      'some regex',
      'another regex',
    ])
...
const compareConfigsExcludingTestRegex = (
  actual: JestSettings,
  expected: JestSettings,
) => {
  // excluding the testRegex key, the rest of the config should be the same.
  const actualEntries = Object.entries(actual.configs[0]).filter(
    ([key]) => key !== 'testRegex',
  )
  const expectedEntries = Object.entries(expected.configs[0]).filter(
    ([key]) => key !== 'testRegex',
  )
...
const createConfigWithTestRegex = (
