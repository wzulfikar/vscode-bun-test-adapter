...
const POSITIVE_WHOLE_NUMBERS = ['1', '123908124']
const NEGATIVE_WHOLE_NUMBERS = ['-1', '-123908124']
const SPECIAL_NUMBERS = ['NaN', 'Infinity', '-Infinity']
const DECIMALS = ['0.123', '1123.1', '-0.0', '-1.1241', '-123.1412']
const ZERO = '0'
...
const JSON_STRINGS = ['someString', 'some"escaped'].map((str) =>
  JSON.stringify(str),
)
...
describe('replacePrintfPatterns', () => {
  describe('replaces %#', () => {
    ...
    it.each(
      PRINTF_PATTERNS.filter(
        (otherPlaceholder) => otherPlaceholder !== placeholder,
      ),
    )('does NOT match other printf patterns (%s)', (otherPlaceholder) => {
      expect(regex.test(otherPlaceholder)).toBeFalse()
    })

    it.each(POSITIVE_WHOLE_NUMBERS)(
      'matches positive whole numbers (%s)',
      (value) => {
        expect(regex.test(value)).toBeTrue()
      },
    )

    it.each(NEGATIVE_WHOLE_NUMBERS)(
      'does NOT match negative numbers (%s)',
      (value) => {
  ...
  ;['%p', '%s', '%o'].forEach((placeholder) =>
    describe(`replaces ${placeholder}`, () => {
      ...
      )('matches other printf patterns (%s)', (otherPlaceholder) => {
  ...
  describe('replaces %j', () => {
    ...
    it.each(OBVIOUS_INVALID_JSON_STRINGS)(
      'does NOT match very obvious invalid strings (%s)',
      (value) => {
        expect(regex.test(value)).toBeFalse()
      },
    )
