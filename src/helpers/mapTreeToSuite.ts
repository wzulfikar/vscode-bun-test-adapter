...
import type { TestInfo, TestSuiteInfo } from 'vscode-test-adapter-api'
...
const mapWorkspaceRootToSuite = (
  rootNode: WorkspaceRootNode,
): TestSuiteInfo | undefined => {
  let projects: ProjectRootNode[]
  if (shouldHideEmptyProjects()) {
    projects = rootNode.projects.filter(
      (p) => p.files.length > 0 || _.some(p.folders, (f) => folderHasFiles(f)),
    )
  } else {
...
const flatMapWorkspaceRootToSuite = ({
  projects,
  id,
  label,
}: WorkspaceRootNode): TestSuiteInfo | undefined => {
  if (!projects.length) {
    return undefined
  }

  const suite = projects.reduce<TestSuiteInfo>(
    (results, project) => {
...
const findDescribeBlocksInNode = (
  ...
  if (isProjectRootNode(node) || isFolderNode(node)) {
    const describesInFolders = _.flatMap(node.folders, (folder) =>
      findDescribeBlocksInNode(folder, results),
    )
    const describesInFiles = _.flatMap(node.files, (file) =>
      findDescribeBlocksInNode(file, results),
    )
...
const shouldHideEmptyProjects = () =>
  vscode.workspace
    ...
    .get<boolean>('hideEmptyProjects') ?? true

const folderHasFiles = (folder: FolderNode): boolean =>
  folder.files.length > 0 || _.some(folder.folders, (f) => folderHasFiles(f))

const mapProjectRootNodeToSuite = ({
  label,
  id,
  files,
  folders,
}: ProjectRootNode): TestSuiteInfo => ({
  children: folders
    .map(mapFolderNodeToTestSuite)
    .concat(files.map(mapFileNodeToTestSuite)),
  id,
  label,
  type: 'suite',
})
...
const mapFolderNodeToTestSuite = (folder: FolderNode): TestSuiteInfo => ({
  children: folder.folders
    .map(mapFolderNodeToTestSuite)
    .concat(folder.files.map(mapFileNodeToTestSuite)),
  id: folder.id,
  label: folder.label,
  type: 'suite',
})

const mapFileNodeToTestSuite = (
  ...
  switch (file.type) {
    case 'file':
      return {
        ...
        children: file.describeBlocks
          .map(
            (d) => mapDescribeBlockToTestSuite(d) as TestSuiteInfo | TestInfo,
          )
    ...
    case 'fileWithParseError':
      return {
        ...
        message: file.error,
        tooltip:
          'Error parsing test file.  This may not be an issue with your code, but check the extension logs for details.',
      }
  }
}

const mapDescribeBlockToTestSuite = (
