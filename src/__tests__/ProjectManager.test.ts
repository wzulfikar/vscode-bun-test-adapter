...
import type { ProjectChangeEvent } from '../repo/types'
import type {
...
describe('ProjectManager tests', () => {
  ...
  test(`Given a ProjectManager instance
    ...
    const testLoaders = testLoaderClass.mock.results.map(
      (x) => x.value as TestLoader,
    )
    ...
    testLoaders.forEach((loader) => {
      const subscriptions = loader.environmentChange as jest.MockedFunction<
        vscode.Event<EnvironmentChangedEvent>
      >
    ...
    const repoParser = await getParser()
    const projectChangeCalls = repoParser.projectChange as jest.MockedFunction<
      vscode.Event<ProjectChangeEvent>
    >
    const projectChangedSubscriptions = projectChangeCalls.mock.results.map(
      (x) => x.value as vscode.Disposable,
    )
    projectChangedSubscriptions.forEach((x) =>
      expect(x.dispose).toHaveBeenCalledTimes(1),
    )
  })
